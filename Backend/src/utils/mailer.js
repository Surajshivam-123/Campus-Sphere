...
/**
 * Send a 6-digit registration OTP to the given email address.
 */
export const sendRegistrationOtpEmail = async (email, otp) => {
  await transporter.sendMail({
    from: `"Campus Sphere" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: "Verify your email for Campus Sphere",
    html: `
      <div style="font-family:sans-serif;max-width:400px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px;">
        <h2 style="color:#1e3a5f;margin-bottom:8px;">Campus Sphere</h2>
        <p style="color:#374151;font-size:14px;">Thank you for starting your registration! Use the OTP below to verify your email address. It is valid for <strong>10 minutes</strong>.</p>
        <div style="font-size:32px;font-weight:700;letter-spacing:8px;color:#d97706;text-align:center;padding:16px 0;">
          ${otp}
        </div>
        <p style="color:#9ca3af;font-size:12px;text-align:center;">If you did not request this verification, please disregard this email.</p>
      </div>
    `,
  });
};
