...
export default function EventChat() {
  ...
  // ── Check access and moderation rights ──────────────────────────────────
  useEffect(() => {
    ...
    const checkAccess = async () => {
      ...
      const [memberRes, participantRes] = await Promise.all([
        fetchWithAuth(`${API_URL}/api/cpsh/members/check/${eventId}`),
        fetchWithAuth(`${API_URL}/api/cpsh/participants/check/${eventId}`),
      ]);

      const memberData = await memberRes.json();
      const participantData = await participantRes.json();

      if (!memberData.success && !participantData.success) {
  ...
  const loadHistory = useCallback(async (page = 1) => {
    const url = `${API_URL}/api/cpsh/event-messages/${eventId}?page=${page}&limit=50`;
    const res = await fetchWithAuth(url);
    const data = await res.json();

    if (res.status === 403) { setAccessDenied(true); setLoadingHistory(false); return null; }
    if (!data) { setLoadingHistory(false); return null; }
  ...
  useEffect(() => {
    ...
    socket.emit("join:event:chat", { eventId, userId: user._id });

    // Remove any stale listeners before adding fresh ones
    socket.off("event:chat:message");
    socket.off("event:chat:deleted");
    socket.off("event:chat:error");
    ...
    const onError = ({ message: errMsg }) => {
      console.error("[event chat error]", errMsg);
    };

    socket.on("event:chat:message", onMessage);
    socket.on("event:chat:deleted", onDeleted);
    socket.on("event:chat:error", onError);

    return () => {
      socket.emit("leave:event:chat", eventId);
      socket.off("event:chat:message", onMessage);
      socket.off("event:chat:deleted", onDeleted);
      socket.off("event:chat:error", onError);
    };
  }, [user, eventId]);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4" style={{ backgroundColor: "var(--color-bg)" }}>
        <p className="text-sm font-medium" style={{ color: "var(--color-error)" }}>
          You must be a member or participant to access this chat.
        </p>
        <button onClick={() => navigate(`/events/${eventId}`)} className="btn-secondary px-4 py-2 text-sm">
          Back to Event
        </button>
      </div>
    );
